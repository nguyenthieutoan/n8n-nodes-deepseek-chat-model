...
describe('LmChatDeepSeek Community Node', () => {
	...
	test('HTTP-level tool filtering via custom fetch configuration', async () => {
		...
		try {
			...
			// Warning user message should be appended to guide the model out of the loop
			expect(sentLoopBody.messages.length).toBe(6);
			expect(sentLoopBody.messages[5].role).toBe('user');
			expect(sentLoopBody.messages[5].content).toContain('vòng lặp gọi công cụ');
